...
# npl = 9
# TiM = np.linspace(0.8,1.2,npl)
# pM = np.zeros((npl,nn))
# for Ti,i in zip(TiM,range(npl)):
#   pM[i] = np.vectorize(p0)(vM,Ti*np.ones(nn))
...
# TTM = np.linspace(0.01,10,nn)
# bM = np.vectorize(beta)(TTM)
...
# Tc1 = 0.8584544774
# nn = 1000
# vM = np.linspace(0.01,10.,nn)
# TM1 = Tc1*np.ones(nn)
# pM1 = np.vectorize(p0)(vM,TM1)

# Tc2 = 0.8711728693
# TM2 = Tc2*np.ones(nn)
# pM2 = np.vectorize(p0)(vM,TM2)

# TM3 =0.85*np.ones(nn)
# pM3 = np.vectorize(p0)(vM,TM3)
# TM4 =0.86*np.ones(nn)
# pM4 = np.vectorize(p0)(vM,TM4)
# TM5 =0.88*np.ones(nn)
# pM5 = np.vectorize(p0)(vM,TM5)
...
# plt.plot(vM,pM5)
# plt.ylim([-5.5,2.5])
# plt.legend(["Tc1", "Tc2", "0.85", "0.86", "0.87"])
# plt.grid()
# plt.show()
...
# nn = 100
# vM = np.linspace(0.01,0.6,nn)
# TM =(Tc1+0.001)*np.ones(nn)
# pM = np.vectorize(p0)(vM,TM)

# plt.figure()
# plt.plot(vM,pM)
# plt.plot(vM,p2(Tc1+0.001)*np.ones(nn))
# plt.plot(vM,p3(Tc1+0.001)*np.ones(nn))
# # plt.plot(vM,pM)
# plt.ylim([-5,-4.8])
# plt.legend(["p","p2","p3"])
# plt.grid()
...
# p01 = p2(T0)
# p02 = p3(T0)
# v12 = fsolver(p0,v0,1,T0,y0=p01)
# v21 = fsolver(p0,1+1e-5,10,T0,y0=p02)

# nn = 100
# vM = np.linspace(0.1,2.5,nn)
# TM =T0*np.ones(nn)
# pM = np.vectorize(p0)(vM,TM)
...
# v1,v2,scs = PhTrSolve(T0,p01,p02,v0,v12,v21,20,eps1=3e-3)
# v1,v2,scs,disM,dpM,v1M,v2M,p1M = PhTrSolve(T0,p01,p02,v0,v12,v21,20,eps1=1e-2,eps2=1e-3)

# v1 = fsolver(p0, v0, v12, T0, p01, drct=-1, eps=1e-3, dx=1e-5)
# v2 = fsolver(p0, v21, 10, T0, p01, drct=1, eps=1e-3, dx=1e-1)

# # v12, v12M = fsolver(p0,0.1,1,Tc1+0.001,y0=p3(Tc1+0.001))
# # v21, v21M = fsolver(p0,v12,1,Tc1+0.001,y0=p2(Tc1+0.001))
...
# nn = 100
# # T0 = Tc2+0.0001
# vM = np.linspace(0.6,1.3,nn)
# TM =T0*np.ones(nn)
# pM = np.vectorize(p0)(vM,TM)

# plt.figure()
# plt.plot(vM,pM)
# plt.plot(v1,p0(v1,T0),'o')
# plt.plot(v2,p0(v1,T0),'o')
# # plt.plot([v1,v2],[p0(v1,T0),p0(v2,T0)])
# plt.ylim([0.986,0.991])
# # plt.legend(["p","p3","p2"])
# plt.grid()
# plt.show()


# v01 = 0.20432141113281263
# v02 = 0.29003125000000013
# TM = T0*np.ones(nn)
# v1M = v01*np.ones(nn)
# v2M = np.linspace(v01,v02,nn)
# yM = np.vectorize(PhTrEq2)(v1M,v2M,TM)
...
# vv, vvM = fsolver(p0,0.14,0.5,0.87,-3.5,drct=-1,eps=1e-7)
...
# nn = 100
# T0 = 0.88
# vM = np.linspace(0.1,2.2,nn)
# TM =T0*np.ones(nn)
# pM = np.vectorize(p0)(vM,TM)

# v1 = fsolver(p0,0.1,1,T0,y0=p1(T0))
# v2 = fsolver(p0,v1,1,T0,y0=p2(T0))
# v3 = fsolver(p0,1,20,T0,y0=p3(T0))
